...
window.addEventListener('load', function() {
    ...
    if (chatBox) {
        ...
        `;
        welcomeMessage.textContent = 'ازيك! اقدر اساعدك ازي بخصوص مادة اساسيات التصميم';
    ...
    const departmentInfo = `قسم تكنولوجيا التعليم بكلية التربية النوعية جامعة المنيا يقدم برنامجًا أكاديميًا شاملًا في مرحلة البكالوريوس مع تخصصين رئيسيين: أخصائي تكنولوجيا التعليم ومعلم الحاسب الآلي. يهدف القسم إلى إعداد كوادر متخصصة في دمج التقنيات الحديثة في العملية التعليمية، وتنمية مهارات الطلاب في استخدام وتصميم الوسائل التعليمية الرقمية. كما يضم القسم نخبة من أعضاء هيئة التدريس ذوي الخبرة في مجالات التعليم الإلكتروني والوسائل التعليمية الرقمية.`;
    const developerInfoAnswer = `جورج هو شاب مصري مهتم بتطوير الويب والأمن السيبراني. يتميز بحب العزلة والتفكير العميق، ويسعى خلال التميز في مجاله التقني.\n\n<strong>المهارات:</strong>\n- Front-End Development (HTML, CSS, JavaScript)\n- Cybersecurity (Bug Bounty Hunting, OWASP Top 10)\n- Python\n- Linux Basics\n- استخدام أدوات مثل PortSwigger\n\n<strong>الاهتمامات:</strong>\n- مهتم بدمج الذكاء الاصطناعي مع الأمن السيبراني.`;
    const sourceInfoAnswer = "أحصل على معلوماتي من قاعدة بيانات مخصصة تحتوي على معلومات دقيقة عن قسم تكنولوجيا التعليم بكلية التربية النوعية، جامعة المنيا، والتي قام بإعدادها جورج عادل.";
    const developerKeywords = ["طورك","مين عملك","مين صممك","المطور","من هو جورج","من هو المطور","who developed you","who created you"];
    const sourceKeywords = ["مصدر معلوماتك","كيف تحصل على المعلومات","source of information","where do you get info"];
    ...
    async function sendMessage() {
        ...
        if (message) {
            ...
            if (previousMessages.has(message)) {
                ...
                conversationHistory.push({role: 'assistant', text: 'لقد قلت هذا من قبل! هل تريد إعادة طرح نفس السؤال؟'});
                chatInput.value = '';
                return;
            }
            ...
            if (developerKeywords.some(k => lowerMsg.includes(k.toLowerCase()))) {
                addMessage(developerInfoAnswer, false);
                conversationHistory.push({role: 'assistant', text: developerInfoAnswer});
                return;
            }
            ...
            if (lowerMsg.includes('قسم تكنولوجيا التعليم') || lowerMsg.includes('تكنولوجيا التعليم')) {
                addMessage(departmentInfo, false);
                conversationHistory.push({role: 'assistant', text: departmentInfo});
                return;
            }
            ...
            try {
                const response = await fetch(apiUrl, {
                    ...
                    body: JSON.stringify({
                        contents: [{ 
                            parts: [{ 
                                text: getSystemPrompt(departmentInfo, conversationHistory) + message 
                            }] 
                        }],
                        generationConfig: { 
                            temperature: 0.7, 
    ...
    function startNewChat() {
        ...
        if (chatBox) {
            ...
            welcomeMessage.style.cssText = `
                ...
                animation: fadeIn 0.3s ease;
            `;
            welcomeMessage.textContent = 'مرحباً! كيف يمكنني مساعدتك في أسئلة المرجعات';
            chatBox.appendChild(welcomeMessage);
        }
